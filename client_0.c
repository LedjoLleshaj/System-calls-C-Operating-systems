...
void operazioni_client0() {
    ...
    if (shm_ptr == NULL)
        shm_ptr = (msg_struct *) shmGet_CreateShmid(shmid, S_IRUSR | S_IWUSR);
    ...
    if (shm_check_ptr == NULL)
        shm_check_ptr = (int *) shmGet_CreateShmid(shm_check_id, S_IRUSR | S_IWUSR);
    ...
    if (fifo1_fd < 0)
        fifo1_fd = create_fifo(PATH_FIFO1, 'w');
    printf("Mi sono collegato alla FIFO 1\n");

    if (fifo2_fd < 0)
        fifo2_fd = create_fifo(PATH_FIFO2, 'w');
